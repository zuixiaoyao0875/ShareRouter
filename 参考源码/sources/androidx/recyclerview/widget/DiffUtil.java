        ...
        while (!arrayList2.isEmpty()) {
            ...
            if (snakeDiffPartial != null) {
                ...
                if (snakeDiffPartial.reverse) {
                    ...
                    range2.oldListEnd = snakeDiffPartial.f65x - 1;
                    range2.newListEnd = snakeDiffPartial.f66y;
                } else {
                    range2.oldListEnd = snakeDiffPartial.f65x;
                ...
                if (snakeDiffPartial.reverse) {
                    if (snakeDiffPartial.removal) {
                        range.oldListStart = snakeDiffPartial.f65x + snakeDiffPartial.size + 1;
                        range.newListStart = snakeDiffPartial.f66y + snakeDiffPartial.size;
                    } else {
                        range.oldListStart = snakeDiffPartial.f65x + snakeDiffPartial.size;
                        range.newListStart = snakeDiffPartial.f66y + snakeDiffPartial.size + 1;
                    }
                } else {
                    range.oldListStart = snakeDiffPartial.f65x + snakeDiffPartial.size;
                    range.newListStart = snakeDiffPartial.f66y + snakeDiffPartial.size;
    ...
    public static class DiffResult {
        ...
        private void findMatchingItems() {
            ...
            for (int size = this.mSnakes.size() - 1; size >= 0; size--) {
                ...
                int i3 = snake.f65x + snake.size;
                int i4 = snake.f66y + snake.size;
        ...
        private boolean findMatchingItem(int i, int i2, int i3, boolean z) {
            ...
            while (i3 >= 0) {
                ...
                int i7 = snake.f65x + snake.size;
                int i8 = snake.f66y + snake.size;
                if (z) {
        ...
        public void dispatchUpdatesTo(ListUpdateCallback listUpdateCallback) {
            ...
            for (int size = this.mSnakes.size() - 1; size >= 0; size--) {
                ...
                for (int i6 = i3 - 1; i6 >= 0; i6--) {
                    if ((this.mOldItemStatuses[snake.f65x + i6] & 31) == 2) {
                        batchingListUpdateCallback.onChanged(snake.f65x + i6, 1, this.mCallback.getChangePayload(snake.f65x + i6, snake.f66y + i6));
                    }
                }
                i = snake.f65x;
                i2 = snake.f66y;
